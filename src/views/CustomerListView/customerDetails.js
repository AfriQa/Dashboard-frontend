...
import "./customer.css";
import { parseString, parsePrice } from "../../helpers/dateHelpers";
      ...
      <Grid container spacing={3}>
        ...
        <Grid item md={6} xs={12}>
          <TextField
            ...
            value={customer.phone? customer.phone : customer.phoneNumber}
          />
        </Grid>
        <Grid item md={6} xs={12}>
          <TextField
        ...
        <Grid item md={4} xs={12}>
          <TextField
            ...
            color="secondary"
            type="text"
            value={new Date(Number(customer.createdAt))}
          ></TextField>
        </Grid>
        <Grid item md={4} xs={12}>
          <TextField
            ...
            <TableBody>
              {orders.map((order) => (
                <TableRow hover key={order.id}>
                  <TableCell>{order.id.slice(order.id.length - 7, order.id.length - 2)}</TableCell>
                  <TableCell>{parseString(order.products)}</TableCell>
                  <TableCell>
                    {moment(order.createdAt).format("DD/MM/YYYY")}
                  </TableCell>
                  <TableCell>{parsePrice(order.products, order.amount)}</TableCell>
...
export default function AlertDialog({ customer, orders }) {
  ...
  return (
    <div className="history">
      ...
      >
        ...
        <DialogContent>
          <History customer={customer} orders={orders} />
        </DialogContent>
        <DialogActions>
